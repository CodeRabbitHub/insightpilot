...
async def get_answer(question=FIXED_QUESTION):
    ...
    given question (default FIXED_QUESTION). Returns (sql, rows).
    Validation runs against the owner-role connection (schema/catalog
    lookups); execution runs separately through execute_sql()'s own
